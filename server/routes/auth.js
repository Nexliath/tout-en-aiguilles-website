...
router.post('/register', async (req, res) => {
  ...
  if (existing) {
    if (!existing.email_verified) {
      // Compte existe mais non vérifié → renvoyer l'email
      ...
      db.prepare('INSERT INTO email_verification_tokens (user_id, token, expires_at) VALUES (?, ?, ?)')
        .run(existing.id, token, tokenExpiresAt());
      await sendVerificationEmail(email, first_name, token, BASE_URL);
  ...
  const token = generateToken();
  db.prepare('INSERT INTO email_verification_tokens (user_id, token, expires_at) VALUES (?, ?, ?)')
    .run(result.lastInsertRowid, token, tokenExpiresAt());
...
router.post('/resend-verification', async (req, res) => {
  ...
  const token = generateToken();
  db.prepare('INSERT INTO email_verification_tokens (user_id, token, expires_at) VALUES (?, ?, ?)')
    .run(user.id, token, tokenExpiresAt());

  try {
    await sendVerificationEmail(email, user.first_name, token, BASE_URL);
  } catch (err) {
    console.error('Erreur envoi email:', err.message);
